...
class ColorGradient
{
    ...
    public function setNumberRange(float $minValue, float $maxValue){
        ...
        $this->minValue = round($minValue, 3, PHP_ROUND_HALF_DOWN);
        $this->maxValue = round($maxValue, 3, PHP_ROUND_HALF_UP);
    }
    ...
    public function colorAt(float $number){
        ...
        $r = $this->formatHex($this->calcHex($number, substr($this->startColor, 0, 2), substr($this->endColor, 0, 2)));
        $g = $this->formatHex($this->calcHex($number, substr($this->startColor, 2, 2), substr($this->endColor, 2, 2)));
        $b = $this->formatHex($this->calcHex($number, substr($this->startColor, 4, 2), substr($this->endColor, 4, 2)));
